...
def get_step_num(StepNum):
    ...
    if StepNum == "":
        print(f"预设步数为空，设置随机步数！")
        return str(random.randint(minStep, maxStep))
    elif '-' not in StepNum:
        print(f"预设步数为{StepNum}，设置固定步数！")
        return StepNum
    elif len(StepArr) >= 2:
        print(f"预设步数为{StepArr[0]}-{StepArr[1]}，设置随机步数！")
        if int(StepArr[0]) <= int(StepArr[1]):
            return str(random.randint(int(StepArr[0]), int(StepArr[1])))
        else:
            return str(random.randint(int(StepArr[1]), int(StepArr[0])))
    else:
        print(f"预设步数无效，设置随机步数！")
        return str(random.randint(minStep, maxStep))
...
def login(PhoneNum, PassWord):
    ...
    Reg = requests.post(RegUrl, data=RegData, headers=Headers, allow_redirects=False)
    try:
    ...
    LogIn = requests.post(LogInUrl, data=LogInData, headers=Headers).json()
    login_token = LogIn["token_info"]["login_token"]
    userid = LogIn["token_info"]["user_id"]
...
def set_step(PhoneNum, PassWord, StepNum):
    ...
    data_json = re.sub(findstep.findall(data_json)[0], StepNum, str(data_json))
...
def main(event, context):
    ...
    if ScheduleTime:
        ...
        print("No ScheduleTime, Run for one-time")
        job()
